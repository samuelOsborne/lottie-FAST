...
export const LottiePlayerControlsTemplate = html`
    <div class="lottie-player-controls" id="player-controls" style="display: ${(x) => (!x.controls ? 'none' : '')}">
        ...
        <div
            class="btn"
            @click="${(x) => x.pauseAnimation()}"
            id="pause-btn"
            style="display: ${(x) => (!x.playing ? 'none' : '')}"
        >
            <svg width="14px" height="14px" viewBox="0 0 24 24">
        ...
        />
        <div class="frame-number" id="frame-number">${(x) => x.currentFrame}</div>
        ...
        <div class="btn" @click="${(x) => x.zoomAnimation()}" style="padding: 10px">
            <svg width="14px" height="14px" viewBox="0 0 24 24" style="display: ${(x) => (!x.isZoomed ? 'none' : '')}">
                <path
